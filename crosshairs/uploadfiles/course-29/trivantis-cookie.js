...
function saveVariable(name,value,days,title,lms) {
  ...
  var encValue = escape( UniEscape( value ) )
  ...
  trivLogMsg( 'saveVariable for ' + name + ' to [' + value + ']' )
  if( titleMgr )
  {
...
function readVariable(name,defval,days,title) {
    ...
    for(i=0;i<ca.length;i++) 
    {
      ...
      if( c.indexOf(myCookie) == 0 )
      {
        ...
        if( vo >= 0 )
        {
          ...
          var valUn = UniUnescape( unescape( val ) )
        
          if( titleMgr )
            titleMgr.setVariable(name,val,days)
          trivLogMsg( 'readVariable for ' + name + ' = [' + valUn + ']' )
          return valUn
        }
      }
    }
  ...
  if( titleMgr ) {
    var res = new String( titleMgr.getVariable(name,escape(UniEscape(defval)),days) )
    defval = UniUnescape( unescape( res ) )
  }
  trivLogMsg( 'readVariable for ' + name + ' = [' + defval + ']' )
  return defval
}
...
function VarUpdateValue() {
  ...
  if( this.cm ) {
    ...
    var titleMgr = getTitleMgrHandle();
    if( titleMgr ) {
      var vv=new String(titleMgr.getVariable(this.name,escape(UniEscape(this.defVal)),this.exp));
      this.value=UniUnescape(unescape(vv));
    }
    ...
    if( this.origAICC ) {
      if( this.bSCORM ) {
        ...
        if( titleMgr ) {
          titleMgr.setVariable(this.name,escape(UniEscape(this.value)),this.exp)
          if( this.name=='cmi.learner_id' ) titleMgr.setVariable('cmi.core.student_id',escape(UniEscape(this.value)),this.exp)
          if( this.name=='cmi.learner_name' ) titleMgr.setVariable('cmi.core.student_name',escape(UniEscape(this.value)),this.exp)
      ...
      if( this.bSCORM ) {
        ...
        if( titleMgr && titleMgr.findVariable( this.name ) != -1 ){
          ...
          if( data == '' ) {
            if( titleMgr ) titleMgr.setVariable(this.name,escape(UniEscape(this.value)),this.exp)
          }
          else {
            var ca = data.split(';')
            for(var i=0;i<ca.length;i++) {
              ...
              if( c.indexOf('=') >= 0 ) {
                ce = c.split('=')
                if( this.name == ce[0] ) this.value = UniUnescape(unescape(ce[1]))
    ...
    if( this.bSCORM ) {
      ...
      if( this.name == 'cmi.core.total_time' || this.name == 'cmi.total_time' ) {
        if( this.aiccframe == 'scorm' ) {
          ...
          if( titleMgr ) titleMgr.setVariable('cmi.core.session_time',escape(UniEscape(this.value)),this.exp)
        }
        else {
          LMSSetValue( 'cmi.session_time', lmsVal )
          if( titleMgr ) titleMgr.setVariable('cmi.session_time',escape(UniEscape(this.value)),this.exp)
        }
      }
      else {
        if( titleMgr ) titleMgr.setVariable(this.name,escape(UniEscape(this.value)),this.exp)
        if( this.aiccgroup ) {
          ...
          var newData= nameEQ + escape(UniEscape(this.value)) + ';'
          var bErr = false;
          var data=new String( LMSGetValue( 'cmi.suspend_data' ) )
          if( data != '' ) {
            var ca = data.split(';');
          ...
          if( UniUnescape(unescape(chkdata)).length < UniUnescape(unescape(newData)).length ) {
            bErr = true;
          }

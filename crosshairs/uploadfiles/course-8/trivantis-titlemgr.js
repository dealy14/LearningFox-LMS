    ...
    if( urlDest != null )
    {
      ...
      if( put )
      {
        if( this.cmVers >= 2 )
        {
          ...
          if( locIdx >= 0 )
            props.addparm( "loc", this.arVars[locIdx].vv );

          props.addparm( "vars", varB.str );
        }
      }
      else
...
TMPr.loadTest = function( tn, pn )
{
  ...
  if( !this.arTests[tIdx].bLoaded )
  {
    ...
    if( !nl && window.ActiveXObject)
    {
      ...
      nl.async="false"
      nl.loadXML(httpReq.responseText)
    }
...
TrivStr.prototype.addparm = function( parm, val )
{
  if( this.str.length > 0 )
    this.str += "&"
  this.str += escape(parm) + "=" + escape(val);
}
...
function getNVStr( nl, tag, dv )
{
  ...
  if( nv && nv.firstChild ) return nv.firstChild.data;
  else return dv;
}
...
{ 
  ...
  if( this.iShowRes )
  {
    ...
    if( this.iShowRes == 10 )
      document.resWind = ObjLayerActionGoToNewWindow( 'trivantistestresults.html?testname='+this.strLoadedName, 'Trivantis_Popup_Result', 'width=528,height=500,scrollbars=1,resizable=1,menubar=0,toolbar=0,location=0,status=0' );
  }
  ...
  if( bSuccess && strDest != null )
  {
    if ( strDest != null && strDest.length > 0 )
    {
      ...
      if( bSuccess && this.bPromptSuc )
      {
        ...
        if( document.resWind != null )
          document.resWind.alert( subTxt );
        else
          alert( subTxt );
      }
    }
...
{ 
  ...
  if( this.bIncVar )
  {
    for( idx = 0; idx < arVars.length; idx++ ) {
      ...
      pl.addparm( this.getVariableSubmitName( idx, arVars ),
                  submitValue );
    }
  }
  return true;
...
{ 
  if( arVars[idx].vn.length > 3 )
  {
    var pN  = arVars[idx].vn.substring(0, 3 ).toLowerCase();
    if( pN == "var" )
      return arVars[idx].vn.substring( 3 );
  }
  return arVars[idx].vn;
}

TTPr.getVariableSubmitValue = function( idx, arVars ) 
{ 
  if( arVars[idx].vv.length > 7 ) 
  {
    var pV = arVars[idx].vv.substring(0, 7 );
    if( pV == "~~f=1~~" ||
        pV == "~~f=2~~" ||
        pV == "~~f=4~~" )
    {
      var slashPos = arVars[idx].vv.indexOf( '|' );
      return arVars[idx].vv.substring( slashPos + 1 );
    }
  }
  else if ( arVars[idx].vv == "~~~null~~~" )
    return "";

  return arVars[idx].vv;
}
...
TQPr.load = function( sid, nl )
{
  ...
  this.corrAns = UniUnescape( getNVStr( nl, 'correctans', this.corrAns ));
  if( this.type == DD || this.type == MT )
  {
    var pos1 = 0
    var pos2
  ...
  this.choices = UniUnescape( getNVStr( nl, 'choices', this.choices ));

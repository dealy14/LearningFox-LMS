...
<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" WIDTH="100%">
  ...
  <TR>
    ...
    <TD bgcolor="#93BEE2"><B><SPAN CLASS="innerl">Modified</TD>
...
$db = new db;
$db->connect();
$db->query("SELECT name,ID,created,modified$topic_sql_extra FROM $obtable ORDER BY $order");
$xm=0;
while($db->getRows())
{ 
	$ID=$db->row("ID");
	...
	?>
	  ...
	  <TR CLASS="bkg" ID="m<?php echo $ID;?>">
		...
		<TD NOWRAP><SPAN CLASS="innerl"><A HREF="import_form.php?id=<?php echo $ID;?>&cn=<?php echo $db->row("name");?>" target="les_tree"><IMG SRC="<?php echo $rimage;?>" BORDER="0" ALIGN="ABSMIDDLE"></A> <?php echo $db->row("name");?></TD>
		
		
		<TD><SPAN CLASS="innerl"><?php echo $db->row("created");?></TD>
		<TD><SPAN CLASS="innerl"><?php echo $db->row("modified");?></TD>

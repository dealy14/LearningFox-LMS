...
//echo "Type: " . $_FILES["file"]["type"] . "hello<br />";
				if ($_FILES["file"]["type"]=="application/zip" || $_FILES["file"]["type"]=="application/x-zip-compressed"){
							 if ($_FILES["file"]["error"] > 0) {
									...
									if(!file_exists("../demo_site/uploadfiles"))
														mkdir("uploadfiles",0777);
									/*if(!file_exists("uploadfiles/".$_POST['cn']))
													mkdir("uploadfiles/".$_POST['cn'],0777);*/
														
									 if (file_exists("../demo_site/uploadfiles/". basename($_FILES["file"]["name"],".zip"))){
    					  							echo '<strong>'.$_FILES["file"]["name"] . " already exists.</.strong> ";
      									}else{
     												 move_uploaded_file($_FILES["file"]["tmp_name"],"../demo_site/uploadfiles/". $_FILES["file"]["name"]);
													$file420 = $_FILES["file"]["name"];
													
													 //$file = getcwd() . "uploadfiles".$_FILES['file']['name'];
													//echo $file;
													// Or:
													$file = $_SERVER['DOCUMENT_ROOT'].'/LMS/demo_site/uploadfiles/'.$_FILES['file']['name'];
												...
												fwrite($fopen, zip_entry_read($zip_entry, zip_entry_filesize($zip_entry)), zip_entry_filesize($zip_entry));
													 ...
													 unzip($file);*/
													 $file1 = "../demo_site/uploadfiles/".$courseid;
													 //die($file1);
													 if(!file_exists($file1))
															mkdir($file1,0777);
															//chdir($file1);
													...
													$file_dir=$_SERVER['DOCUMENT_ROOT']."/LMS/demo_site/uploadfiles/".$courseid;
...
//$z=0;
    foreach($xmlObj->children() as $child) {
		...
		$metapath=$_SERVER['DOCUMENT_ROOT']."/LMS/demo_site/uploadfiles/".$courseid."/".$pre->location;
...
displayChildrenRecursive($sitemap);
				...
				$str='insert into course set created="'.date("y/m/d").'",name="'.$task.'",type="wbt",course_type="'.$_POST['course_type'].'",folder_name="/LMS/demo_site/uploadfiles/'.$courseid.'",course_id="'.$courseid.'",cmi_credit="'.$_POST['radio_credit'].'",sco_version="'.$sco_version.'",keyword="'.$keyword.'",description2="'.$desc.'",catalog_name="'.$catalog_name.'",catalog_entry="'.$catalog_entry.'",link=""';

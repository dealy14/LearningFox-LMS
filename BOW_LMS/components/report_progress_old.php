...
?>
<IMG SRC="images/report_progress.gif" BORDER="0" ALIGN="ABSMIDDLE"> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Report was run on: <?echo date(ymd);?>
<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0"><TR><TD BGCOLOR="#CCCCCC">
<TABLE BORDER="0" CELLSPACING="1" CELLPADDING="3" WIDTH="100%">
<?
hTop("index.php?section=reports&report=testresults&course_ID=$course_ID&sid=$sid","Last Name|lname,First Name|fname,Register Date|date_ofreg,Course Status|course_status,Last Lesson|lesson,Last Topic|topic,Last Login|last_usage,Start Date|start_date,Average Score|total_score",$order,$direction);
...
$extr="ORDER BY $order $direction";
	...
	<TR>
	  ...
	  <TD BGCOLOR="#FFFFFF" VALIGN=TOP><FONT FACE="VERDANA" SIZE="1"><?=$db->row("course_status")?></TD>
	  <TD BGCOLOR="#FFFFFF" VALIGN=TOP><FONT FACE="VERDANA" SIZE="1"><?=$db->row("lesson")?></TD>
	  <TD BGCOLOR="#FFFFFF" VALIGN=TOP><FONT FACE="VERDANA" SIZE="1"><?=$db->row("topic")?></TD>
	  <TD BGCOLOR="#FFFFFF" VALIGN=TOP><FONT FACE="VERDANA" SIZE="1"><?=$db->row("last_usage")?></TD>
	  <TD BGCOLOR="#FFFFFF" VALIGN=TOP><FONT FACE="VERDANA" SIZE="1"><?=$db->row("start_date")?></TD>	  
	  <TD BGCOLOR="#FFFFFF" VALIGN=TOP><FONT FACE="VERDANA" SIZE="1"><?=$db->row("total_score")?></TD>
	</TR>
	<?
	}
?>

...
if($lms_unique=="on")
{
  ...
  $xx++;
    ...
    if($xx>=1)
    {
    echo"<B>Sorry!</B> - someone already has that username, please choose another.";
    $reg_unique="no";
    }
}

//check against orgIDs;
if($lms_orgID=="on")
{
  $db = new db;
  $db->connect();
  $db->query("SELECT ID FROM orgs WHERE org_ID='$orgID'");
  $xx=0;
  while($db->getRows())
  { 
  $xx++;
    ...
    if($xx<1)
    {
    echo"<BR><B>Sorry!</B> - That organization ID is not recognized.";
    $reg_unique="no";
    }
}
...
if($reg_unique!="no")
{
  ...
  $rname=$db->row("field_name");
	  if($rname=="user_group")
	  {
	  $rval=explode("_",$$rname);
	  $sqlb.=$rname.",";
	  $sqlc.="'".$rval[0]."',";
	  }
	  else if($rname=="user_subgroup")
	  {
	  $rval2=explode("_",$user_group);
	  $sqlb.=$rname.",";
	  $sqlc.="'".$rval2[1]."',";	  
	  }
	  else
  ...
  $sql = $sqla.$sqlb."userlevel,date_of_reg,date_of_mod)\nVALUES(".$sqlc."'$lms_default_userlevel','$date_of_reg','$date_of_mod')";

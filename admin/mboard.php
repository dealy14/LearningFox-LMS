...
require_once('../site_conf/components/board/settings.php');
...
<div id="container2">
	...
	<div id="main">
		...
		<?php //include 'includes/header.php'; 
			//print $count.$settings[extension].'<br />';
		...
		<div id="main-content">
			...
			//include_once '../site_conf/components/board/threads.txt';
			$test = '../site_conf/components/board/threads.txt';
			$t1 = file_get_contents($test);
			//$zone = preg_replace('/index.php/','../demo_site/index.php',$t1);
			
			/*preg_match('@count=(.*?)">@',$t1,$pzone1);
			preg_match_all('@count=(.*?)">@',$t1,$pzone);
			$getnum = preg_replace('/&section=msg&/','.html?',$pzone1[1]);*/
			/*for($i=0;$i<count($pzone[1]);$i++){
				print $pzone[1][$i].'<br />';
				
			}*/
		 $zone = preg_replace('/index.php/','../site_conf/components/board/msg/',$t1);
$zone1 = preg_replace('/\?count=/','',$zone);
//echo $zone1;
$zone2 = preg_replace('/\&section=/','.html?admin=yes&section=',$zone1);
			print $zone2;
			$ezone = explode('=', $zone);
			$msgnum = preg_replace('/">/','',$ezone[4]);
			...
			<input type="hidden" value="<?php print $sid; ?>" name="sid" id="sid" />
...
for ($i=0;$i<=count($threads);$i++) {
    if(strstr($threads[$i],'<!--o '.$orig_id.'-->'))
		...
		$threads[$i] .=   "<!--s $count--><ul><li><a href=\"index.php?section=msg&sid=".$_GET['sid']."&count=".$count."\">$subject</a> - <b>$name</b> <i>$date</i>\n";
        $threads[$i] .= "<!--o $count--> (0)\n";
...
for ($i=0;$i<=count($filecontent);$i++) {
    if(preg_match("/<!-- zacni -->/",$filecontent[$i]))
        {
        $filecontent[$i] = "<!-- zacni -->\n<!--s $count--><li><a href=\"index.php?section=msg&sid=".$_GET['sid']."&count=".$count."\">$subject</a> - <b>$name</b> <i>$date</i></li>\n";
        break;
        }
}
...
<link href='.$settings['mboard_url2'].'site_conf/components/board/styleb.css" type="text/css" rel="stylesheet">
...
Submitted by '.$name.' '.$mail.' on '.$date.' '.$other.' ==='.$settings['mboard_url2'];
...
<p><b>Message=='.$settings['mboard_url2'].'</b>:</p>
...
//$content.=include '../includes/footer.php';
$content.='</div>
</body>
</html>';
...
$addline .= "<!--s $count--><ul><li><a href=\"index.php?section=msg&sid=".$_GET['sid']."&count=".$count."\">$subject</a> - <b>$name</b> <i>$date</i>\n";
$addline .= "<!--o $count--> (0)\n";
...
<div id="container">
	<div id="main">
		...
		<div id="main-content">
			...
			<p>&nbsp;</p>
			<p>&nbsp;</p>
			<form action="<?php print $settings['mboard_url2']; ?>site_conf/components/board/mboard.php?sid=<?php print $_GET['sid']; ?>" method="POST"><input type="hidden" name="a" value="confirmdelete">
...
<!--[if IE 5]><style type="text/css">@import "styleIE.css";</style><![endif]-->
<!--[if IE 6]><style type="text/css">@import "styleIE.css";</style><![endif]-->
<!--[if IE 7]><style type="text/css">@import "styleIE.css";</style><![endif]-->
<script type="text/javascript" src="js/formcheck.js"></script></head>
...
<body>
';
include_once '../site_conf/components/board/header.txt';
}
...
<div id="container2">
	<div id="main">
		...
		<div id="main-content">
			<?php //include 'includes/left-nav.php'; ?>
			<div id="main-content-b">
			<p align="center" class="title"><b>Anti-SPAM check</b></p>
			<div align="center"><center>

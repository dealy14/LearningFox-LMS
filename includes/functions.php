...
function generate_dropdown_control($name,$assoc_array_options,$selected_key,$styles="")
{
	echo "<select name=\"$name\" $styles>";
	foreach ($assoc_array_options as $key_value => $description) {
		echo "<option value=\"$key_value\"";
		if ($selected_key == $key_value) { echo " selected"; }
		echo ">$key_value - $description</option>";
	 }
	echo "</select>";
}
...
function makeFieldEdit($field_name,$value)
{
	...
	elseif (strtolower($field_name)=="password")
	{
